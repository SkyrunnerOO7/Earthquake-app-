    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            for(int i = 0; i < earthquakeArray.length(); i++){
                ...
                //the part where we split the location into two
                //Offset location and primary location
                ...
                if(location.contains("km")){
                    split = location.indexOf("of");
                    split += 3;

                    offsetLocation = location.substring(0, split);
                }else{
                    offsetLocation = "Near the";
                }
                primaryLocation = location.substring(split);


                //Making the magnitude double
                //so that it can only show upto one decimal point
                DecimalFormat formatter = new DecimalFormat("0.0");
                formatter.format(magnitude);



                //long code for he below code
//                Earthquake earthquake = new Earthquake(magnitude, location, time);
//                earthquakes.add(earthquake);
                earthquakes.add(new Earthquake(magnitude, offsetLocation, primaryLocation, time));
            }
